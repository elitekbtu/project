...
import { Heart, ShoppingBag, ChevronLeft, MessageSquare, Trash2, Minus, Plus, Store, Package } from 'lucide-react'
...
const ShopItemDetail = () => {
  const { id, moderatorId } = useParams<{ id: string; moderatorId: string }>()
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [detailResp, similarResp, shopData] = await Promise.all([
          api.get<Item>(`/api/items/${id}`),
          api.get<Item[]>(`/api/items/${id}/similar`),
          shopsApi.getShop(parseInt(moderatorId))
        ])
  ...
  let pageTitle = item ? `${item.name}${shop ? ' — ' + shop.name : ''} — TRC` : 'Товар — TRC';
  let pageDesc = item ? `${item.name}${item.brand ? ' от ' + item.brand : ''}${shop ? ' в магазине ' + shop.name : ''}. ${item.description || ''}` : 'Товар на платформе TRC.';
  let pageKeywords = item ? `товар, ${item.name}, ${item.brand || ''}, ${item.category || ''}, ${shop ? shop.name : ''}, TRC` : 'товар, TRC';
  let pageImage = item && item.image_url ? item.image_url : undefined;
  let pagePrice = displayedPrice;
  ...
  return (
    <>
      <Helmet>
        ...
        {pagePrice && <meta property="product:price:amount" content={String(pagePrice)} />}
        {pagePrice && <meta property="product:price:currency" content="KZT" />}
      </Helmet>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
              ...
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">Модератор: {shop.moderator_name}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              {similar.map((it) => (
                ...
                >
                  <Card className="group overflow-hidden transition-all hover:shadow-lg border-0 shadow-sm">
                    <Link to={`/shops/${moderatorId}/items/${it.id}`} className="block">
