...
const ItemDetail = () => {
  ...
  return (
    <>
      {item && (
        <Helmet>
          <title>{item.name} — Купить в TRC</title>
          <meta name="description" content={item.description || 'Купить модную одежду в TRC. Большой выбор, быстрая доставка.'} />
          <meta name="keywords" content={`мода, одежда, ${item.brand || ''}, ${item.category || ''}, купить`} />
